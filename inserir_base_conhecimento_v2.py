#!/usr/bin/env python3
"""Insert all 64 knowledge base responses into Directus - FIXED VERSION"""
...
# ORIENTAÇÃO (15)
for item in [
    ("ORIENTACAO", "Que horas é o culto de domingo?", "O culto de domingo na Igreja Central é às 9h (manhã) e 19h (noite). Venha participar! Deus tem uma palavra para voce.", 95),
    ("ORIENTACAO", "Como faço para entregar o dízimo?", "O dízimo pode ser entregue na sua igreja local durante os cultos, ou por transferência bancária. Consulte seu pastor para os dados da conta oficial da IURD Bolivia. 'Honra ao Senhor com os teus bens' (Provérbios 3:9)", 95),
    ...
    ("ORIENTACAO", "Como faço para ofertar online?", "Você pode ofertar através da conta oficial da IURD Bolivia. Consulte seu pastor para os dados bancários. 'Cada um dê conforme propôs no seu coração' (2 Coríntios 9:7)", 95),
...
# TESTEMUNHO (8)
for item in [
    ("TESTEMUNHO", "Deus abriu uma porta de trabalho pra mim!", "Gloria a Deus! Que testemunho lindo! O Senhor é fiel. Compartilhe esse testemunho no próximo culto!", 95),
    ...
    ("TESTEMUNHO", "Fui liberto do vício", "Gloria a Deus! Se o Filho vos libertar, verdadeiramente sereis livres (João 8:36). Você é uma prova viva do poder de Deus!", 95),
    ...
    ("TESTEMUNHO", "Minha mãe se converteu!", "Aleluia! Os céus estão em festa! Ha alegria diante dos anjos de Deus por um pecador que se arrepende (Lucas 15:10).", 95),
]:
    respostas.append(item)
...
for item in [
    ...
    ("FALAR_COM_ALGUEM", "Preciso de aconselhamento pastoral", "Otimo! Vamos agendar um horário. Informe seu nome, telefone e disponibilidade. Um pastor retornará em breve.", 90),
...
# INFORMAÇÃO (15)
for item in [
    ("INFORMACAO", "Qual o endereço da igreja central?", "A Igreja Central IURD Bolivia fica em Sopocachi, La Paz. Cultos: domingo 9h e 19h, quarta 19h.", 95),
    ...
    ("INFORMACAO", "Qual a programação de TV de hoje?", "Programação TV IURD hoje: 6h Oracion Manhana, 9h Culto, 12h Palabra de Fe, 15h Terapia del Amor, 18h Corrente Diaria, 20h Encuentro con Dios.", 95),
    ("INFORMACAO", "Qual a programação da rádio?", "Radio IURD: programação das 6h às 22h com Oracion Manhana, Palabra de Fe, Reunion de la Sanidad e muito mais. Sintonize!", 95),
    ...
    ("INFORMACAO", "Tem como assistir o culto online?", "Sim! Transmitimos ao vivo pelo Facebook e YouTube da IURD Bolivia. Siga @iurdbolivia!", 95),
    ("INFORMACAO", "Qual o horário da corrente de oração?", "A Corrente Diaria é transmitida todos os dias às 18h pela TV e Radio IURD. Participe!", 95),
]:
    respostas.append(item)
...
count = 0
for cat, perg, resp_t, conf in respostas:
    item = {
        "categoria": cat,
    ...
    try:
        ...
        if e.code == 204:
            ...
            print(f"  ERRO no item {count+1}: HTTP {e.code}")
        if count % 10 == 0:
            print(f"  Inseridos {count}...")

print(f"\n OK TOTAL: {count} respostas inseridas com sucesso!")
